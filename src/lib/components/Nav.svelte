<script lang='ts'>
    ...
    onMount(() => {
        if (browser) {
            ...
            if (nameInput) {
                nameAutocomplete = new google.maps.places.Autocomplete(nameInput as HTMLInputElement, nameOptions) as CustomAutocomplete;
                nameAutocomplete.inputId = 'nameInputElement';
                nameAutocomplete.addListener('place_changed', fillIn);
            }
            if (locationInput) {
                locationAutocomplete = new google.maps.places.Autocomplete(locationInput as HTMLInputElement, locationOptions) as CustomAutocomplete;
...
<div class="{isHomePage? `bg-none border-b-0 border-b-transparent` : `bg-white border-b-black border-b-2`}  z-20 p-[1.25rem] flex fixed top-0 w-full h-36 md:h-24  justify-end items-end md:items-start transform transition-all duration-1000 ease-in-out">
